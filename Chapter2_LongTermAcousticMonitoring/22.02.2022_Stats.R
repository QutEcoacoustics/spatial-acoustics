...
# Daily model (RFClass ~ vars) ----

df_daily <- dplyr::select(df, RFclass, general_category, period, Date, week_day, Recording_time,  moon_illu, TempOut, HumOut, Rain) %>% 
  na.exclude(.) %>% 
  droplevels(.)

## General category; climatic vars + moon ----

rf_general_daily <- randomForest(RFclass ~ moon_illu + TempOut + HumOut + Rain + week_day + period, data = df_daily, importance = T, proximity = T)

print(rf_general_daily)

varImpPlot(rf_general_daily)

### Optimising

importance <- as.data.frame(importance(rf_general_daily)) %>%
  filter(., MeanDecreaseAccuracy >= 0) %>%
  row.names(.)

model_data <- dplyr::select(df_daily, RFclass, all_of(importance)) %>%
  droplevels(.)
