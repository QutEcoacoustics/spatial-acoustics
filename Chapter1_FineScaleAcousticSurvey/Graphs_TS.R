...
# complete_ts <- read.csv(getDataPath("STSC", "Results", data, point_name, paste(data, point_name, ".csv", sep = ""))) %>% 
#   rename(., position = X)
...
plot_ts <- select(complete_ts, all_of(index_name), position, day, time, ResultMinute) %>% 
  filter(day == "10") %>% 
  droplevels(.)
...
ggplot(plot_ts, aes(x = position, y = EventsPerSecond)) +
  ...
  geom_vline(xintercept = c(13441, 14161), linetype = "dotted") +
...
ggplot(plot_ts, aes(x = position, y = TemporalEntropy)) +
  geom_line(aes(colour = index_value), colour = "grey") +
  geom_line(data = plot_motif, aes(x = position, y = index_value, colour = reference)) +
  #geom_line(data = plot_match, aes(x = position, y = Index, colour = number)) + 
  scale_linetype_manual(values = "dotted") +
