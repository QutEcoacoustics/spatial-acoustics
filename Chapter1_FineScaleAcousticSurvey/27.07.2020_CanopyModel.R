...
lm.global <- lm(Ndsi ~ CanopyCover + SubcanopyHeight + AVERAGE_NT_DIST + AVERAGE_NS_DIST + NDVI_AVERAGE, data = df)
...
lm.bp <- bptest(Ndsi ~ CanopyCover + SubcanopyHeight + AVERAGE_NT_DIST + AVERAGE_NS_DIST + NDVI_AVERAGE, data = spatialdf, studentize = T)
...
#Global Model#
#Selecting bandwidth#
BD_sel <- bw.gwr(Ndsi ~ CanopyCover + SubcanopyHeight + AVERAGE_NT_DIST + AVERAGE_NS_DIST + NDVI_AVERAGE, data = spatialdf, approach = "AIC", kernel = "tricube", adaptive = F, dMat = dist_matrix)
...
gwr.res <- gwr.basic(Ndsi ~ CanopyCover + SubcanopyHeight + AVERAGE_NT_DIST + AVERAGE_NS_DIST + NDVI_AVERAGE, data = spatialdf,  bw = BD_sel, adaptive = F, kernel = "tricube")
summary(gwr.res$lm)
...
ggplot(model, aes(x = as.factor(coords.x2), y = Local_R2)) +
  ...
  labs(title = "Local R2 values per point - Canopy and Ndsi Model", x = "Point", y = "Local R2", fill = "Day period") +
  ggsave(getDataPath("Figures", "05.08.2020_gwrr2valuespersite_canopymodel_Ndsi.tiff"))
...
ggplot(df_gather, aes(x = as.factor(coords.x2), y = coefficient_value, fill = coefficient_name)) +
  ...
  labs(title = "Coefficients per point - Canopy and Ndsi model", x = "Point", y = "Coefficients Value", fill = "Variables") +
ggsave(getDataPath("Figures", "05.08.2020_Coefficients_canopymodel_Ndsi.tiff"))
