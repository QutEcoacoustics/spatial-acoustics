...
df_filtered <- filter(df, categorical_time_4groups == "night")

#Linear Model
global_model <- lm(AcousticComplexity ~ CanopyCover + CanopyHeight + SubcanopyHeight + Slope + Aspect + Elevation + AVERAGE_NT_DIST + AVERAGE_NS_DIST + AVERAGE_GC_NG + AVERAGE_GC_NF + AVERAGE_GC_SH, data = df)
...
df_residuals <- cbind(df, global_model$residuals)
...
moran.test(df_residuals$`global_model$residuals`, points.dist.inv) #p significant: the process is not random, there is a spatial component to it; positive observed z score: dataset is more spatially clustered than expected by chance.
