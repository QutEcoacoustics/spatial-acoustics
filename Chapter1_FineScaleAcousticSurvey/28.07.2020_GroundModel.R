...
DeVar <- "EntropyOfVarianceSpectrum"

#Setting indepentendent variables
IndVar <- c("AVERAGE_GC_NF", "NDVI_AVERAGE", "AVERAGE_GC_NG", "AVERAGE_GC_SH")

lm.global <- lm(DeVar ~ IndVar, data = df)


lm.global <- lm(EntropyOfVarianceSpectrum~ AVERAGE_GC_NG + AVERAGE_GC_NF + AVERAGE_GC_SH + NDVI_AVERAGE, data = df)
...
lm.bp <- bptest(EntropyOfVarianceSpectrum~ AVERAGE_GC_NG + AVERAGE_GC_NF + AVERAGE_GC_SH + NDVI_AVERAGE, data = spatialdf, studentize = T)
...
#Global Model#
#Selecting bandwidth#
BD_sel <- bw.gwr(EntropyOfVarianceSpectrum ~ AVERAGE_GC_NG + AVERAGE_GC_NF + AVERAGE_GC_SH + NDVI_AVERAGE, data = spatialdf, approach = "AIC", kernel = "tricube", adaptive = F, dMat = dist_matrix)
...
gwr.res <- gwr.basic(EntropyOfVarianceSpectrum ~  AVERAGE_GC_NG + AVERAGE_GC_NF + AVERAGE_GC_SH + NDVI_AVERAGE, data = spatialdf, bw = BD_sel, adaptive = F, kernel = "tricube")
summary(gwr.res$lm)
...
ggplot(model, aes(x = as.factor(coords.x2), y = Local_R2)) +
  ...
  labs(title = "Local R2 values per point - Ground and and EntropyOfVarianceSpectrum", x = "Point", y = "Local R2", fill = "Day period") +
  ggsave(getDataPath("Figures", "07.08.2020_gwrr2valuespersite_groundmodel_EntropyOfVarianceSpectrum.tiff"))

df_gather <- gather(model_grouped[,1:6], -c(coords.x1, coords.x2), key = "coefficient_name", value = "coefficient_value")
...
ggplot(df_gather, aes(x = as.factor(coords.x2), y = coefficient_value, fill = coefficient_name)) +
  ...
  labs(title = "Coefficients per point - Ground and EntropyOfVarianceSpectrumModel", x = "Point", y = "Coefficients Value", fill = "Variables") +
  ggsave(getDataPath("Figures", "07.08.2020_Coefficients_groundmodel_EntropyOfVarianceSpectrum.tiff"))
