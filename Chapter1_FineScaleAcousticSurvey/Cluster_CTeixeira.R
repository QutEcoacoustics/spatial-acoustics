...
summary(cluster_summary_channel1_df)

hopkins(cluster_summary_channel1_df[8:14], n = 17714)

pv <- pvclust(cluster_summary_channel1_df[8:14], method.hclust = "ward.D", method.dist = "correlation", nboot = 5000, iseed = 123, parallel = T)
pv$edges

clValid(cluster_summary_channel1_df[8:14], nClust = 2:20, clMethods = "hierarchical", validation = c("stability", "internal"), method = "ward")

library(mclust)
mc <- Mclust(cluster_summary_channel1_df[8:14])
