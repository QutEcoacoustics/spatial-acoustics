...
complete_ts %>% select(., 1:4) %>% 
  ...
  ggsave(getDataPath("Results", point, "indicespertime_20200922.jpg"))
...
write.csv(ts, getDataPath("Results", point, paste(point, index, "motif_20200922.csv", sep = "_")))
...
write.csv(ts, getDataPath("Results", point, paste(point, index, "match_20200922.csv", sep = "_")))
