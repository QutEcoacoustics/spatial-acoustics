...
point <- "SERF"
batch <- "20153"
filename_complete_ts <- paste(point, batch, ".csv", sep = "")
...
complete_ts %>% select(., 1:4) %>% 
  ...
  ggsave(getDataPath("Results", point, "indicespertime.jpg"))


parameters <- read.csv(getDataPath("Results", point, "motif_info.csv"))
...
ts <- complete_ts %>%
  mutate(., point = point) %>% 
  mutate(., batch = batch) %>% 
  select(., 1, all_of(index_select), 5:13) %>% 
  mutate(., motif = NA) %>% 
  mutate(., distance = NA) %>% 
  mutate(., length = NA) %>% 
...
write.csv(ts, getDataPath("Results", point, paste(point, index, batch, "motif.csv", sep = "_")))
...
write.csv(ts, getDataPath("Results", point, paste(point, index, batch, "match.csv", sep = "_")))
...
ggplot(plot_ts, aes(x = position, y = Index)) +
  ...
  ggsave(getDataPath("Results", point, "Figures", paste(index, batch, "ts_motifs_matches.jpg", sep = "")))
