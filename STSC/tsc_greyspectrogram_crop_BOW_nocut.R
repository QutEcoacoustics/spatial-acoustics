...
for (row in 1:nrow(cluster_results)) {
  ...
  image_read(.) %>% 
    ...
    image_write(., getDataPath("Chapter1_FineScaleAcousticSurvey", "STSC_GreySpectrograms", "all_motif", paste(cluster_results$fid_what[row], cluster_results$image_file[row], sep = "_")))
}
