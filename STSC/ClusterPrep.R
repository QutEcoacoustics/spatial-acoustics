...
list_matchfiles <- list.files(getDataPath("STSC", "Results"), pattern = "*_match_09102020.csv", recursive = T)
list_motiffiles <- list.files(getDataPath("STSC", "Results"), pattern = "*_motif_09102020.csv", recursive = T)
...
df <- rbind(output_match, output_motif) %>% 
  write.csv(., "C:/Users/n10393021/OneDrive - Queensland University of Technology/Documents/PhD/Project/STSC/Results/matchmotif_bind.csv")
...
for (file in list_motiffiles) {
  file_result <- read.csv(getDataPath("STSC", "Results", file)) %>%
    ...
    rename(., index_value = Index) %>%
    mutate(., id = paste(basename(file) %>%
                           gsub(pattern = "_motif_09102020.csv", replacement = ""), fid_what, sep = "_")) %>%
    dplyr::select(., position, index_value, FileName, location, rec, point, date, time, ResultStartSeconds, ResultMinute, FID, distance, length, reference, id, fid_what)
...
for (file in list_matchfiles) {
  file_result <- read.csv(getDataPath("STSC", "Results", file)) %>%
    ...
    rename(., index_value = Index) %>%
    mutate(., id = paste(basename(file) %>%
                           gsub(pattern = "_match_09102020.csv", replacement = ""), fid_what, sep = "_")) %>%
    dplyr::select(., position, index_value, FileName, location, rec, point, date, time, ResultStartSeconds, ResultMinute, FID, distance, length, reference, id, fid_what)
...
indices <- rbind(match_complete, motif_complete) %>% 
  write.csv(getDataPath("STSC", "Results", "matchmotif_bow_complete.csv"))
# 
