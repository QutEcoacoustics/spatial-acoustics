...
motif_result_ <- read.csv(getDataPath(chapter, "27.02.2021_CompleteData.csv")) %>%
  ...
  # select(1:21) %>%
  mutate(image_file = paste(location, "_", "REC0", RECORDER, "__", "ACI-ENT-EVN", ".png", sep = ""))

motif_result <- filter(motif_result_, point == sensor_point)

all_motifs <- motif_result %>% 
  # filter(., index == "ACI") %>%
  group_by(.$date) %>% 
  mutate(day = cur_group_id())

fcs_img <- list.files(getDataPath("Fieldwork_Bowra", "Aug2019_concatfiles"), pattern = all_motifs$image_file[1], recursive = T, full.names = T) %>%
  load.image(.)

for (row in 1:nrow(all_motifs)) {
  # %>% 
  fcs_img <- draw_rect(im = fcs_img, x0 = ((((all_motifs$day[row]*24)-24)+all_motifs$hour[row]+1)*all_motifs$ResultMinute[row]), y0 = 50, x1 = all_motifs$length[row]-(1-((((all_motifs$day[row]*24)-24)+all_motifs$hour[row]+1)*all_motifs$ResultMinute[row])), y1 = 306, opacity = 0.25, color =	"#FFFFFF")
  
  
}


save.image(fcs_img, getDataPath(chapter, "Figures", "Spect_tagged", paste(all_motifs$image_file[1], "_all.png", sep = "")))


#FCS - SPLIT BIRDS VS INSECTS
...
#   merge(., field_data, by.x = "point", by.y = "Point", all.x = T) %>%
#   # select(1:21) %>%
#   mutate(image_file = paste(location, "_", "REC", RECORDER, "__", "ACI-ENT-EVN", ".png", sep = ""))
# 
# motif_result <- filter(motif_result_, point == sensor_point)
# 
# all_motifs <- motif_result %>%
...
# for (row in 1:nrow(all_motifs)) {
#   # %>%
#   fcs_img <- draw_rect(im = fcs_img, x0 = ((((all_motifs$day[row]*24)-24)+all_motifs$hour[row]+1)*all_motifs$ResultMinute[row]), y0 = 50, x1 = all_motifs$length[row]-(1-((((all_motifs$day[row]*24)-24)+all_motifs$hour[row]+1)*all_motifs$ResultMinute[row])), y1 = 306, opacity = 0.3, color = case_when(all_motifs$class_model[row] == "bird" ~ "#00FFFF",
#                                                                                                                                                                                                                                                                                                             all_motifs$class_model[row] == "insect" ~ "#FF4500",
#                                                                                                                                                                                                                                                                                                             TRUE ~ 	"#FFFFFF"))
# 
# 
# }
# 
# 
# save.image(fcs_img, getDataPath(chapter, "Figures", "Spect_tagged", paste(all_motifs$image_file[1], ".png", sep = "")))
