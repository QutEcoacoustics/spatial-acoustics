...
#Global Model#
#Selecting bandwidth#
# BD_sel <- bw.ggwr(n ~ bvg_char, data = spatial, approach = "AIC", kernel = "tricube", adaptive = F, dMat = dist_matrix)
...
df_gather <- pivot_longer(model_grouped, cols = c(2:10), names_to = "coefficient_name", values_to = "coefficient_value") %>% 
...
ggplot(df_gather, aes(x = site, y = coefficient_value, fill = coefficient_name)) +
  ...
  ggsave(getDataPath("Figures", "02.05.2022_Coefficients_globalmodel.tiff"))
