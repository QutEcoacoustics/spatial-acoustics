...
data_og <- read.csv(getDataPath("09.05.2022_data.csv")) %>% 
  ...
  dplyr::select(site, month, period, n, RFclass, everything(), -c(X, X.1, date_r, Recording_time, avg_rain_previous_months, future_ndvi, avg_temp_previous_months, day, week, ID.x, point)) %>%
  # filter(RFclass == set.group) %>% 
  group_by(site, month, RFclass, period) %>% 
  mutate(n = n()) %>% 
  distinct()
...
(tsp <- ggplot(data = data_og,
                mapping = aes(x = as.integer(month), y = n, group = site)))
...
#After checking the time series, it is clear that tags birdfroginsect don't have more than 5 motifs across sites, therefore they will be excluded from analysis from now on - tags kept = bird, birdinsect, insect

data_og <- filter(data_og, RFclass == "bird" | RFclass == "birdfrog" | RFclass == "birdfroginsect" | RFclass == "birdinsect") %>% 
  droplevels()

(tsp <- ggplot(data = data_og,
               mapping = aes(x = month, y = n, group = site)))
(tsp + geom_line(color = "blue") + 
    gghighlight(max(n) > 5, use_direct_label = FALSE) +
    labs(title= paste(" "), x="Date", y="Number of motifs") +
    theme_classic() +
...
(map <- spatial %>% #filter(month == 8) %>% 
  tm_shape(.) +
  tm_bubbles(size = 1, col = "n") +
  #tm_tiles(aus_map) +
  tm_facets(by = "month", ncol = 1) +
  #tm_borders(aus_map)
  tm_layout(bg.color = "white", # change background colour
          legend.outside = TRUE, # legend outside
          ...
          legend.height = 1,
          panel.label.size = 2,
          main.title = "Bird/Bird Frog/Bird Insect"))
...
anim <- tmap_animation(map, width = 120, height = 120, delay = 100, loop = T)
...
global_model <- lm(n ~ cleared_3k + natural_cover_325 + cleared_3k:contag_landscape_3k + bvg + avg_rain_previous_years, data = data_og)
...
lm.bp <- bptest(n ~ soil_3k + urban_3k + water_3k + cleared_3k + natural_cover_3k + contag_landscape_3k + np_landscape_3k + tca_landscape_3k + soil_325 + urban_325 + water_325 + cleared_325 + natural_cover_325 + contag_landscape_325 + np_landscape_325 + tca_landscape_325 + ndvi_mean + ebi_max + ndwi_mean + temp_max + temp_min + rain_value + moon_illu + bvg + avg_rain_previous_years + month, data = data_og, studentize = T)
...
IndVar <- c("natural_cover_3k", "contag_landscape_3k", "np_landscape_3k", "ndvi_mean", "ebi_max", "ndwi_mean", "temp_max", "rain_value", "moon_illu")



spatial <- as(spatial, Class = "Spatial")

dist_matrix <- gw.dist(dp.locat = coordinates(spatial))
...
#Global Model#
#Selecting bandwidth#
# BD_sel <- bw.ggwr(n ~ avg_rain_previous_years, data = spatial, approach = "AIC", kernel = "tricube", adaptive = F, dMat = dist_matrix)





BD_sel <- bw.ggwr(n ~ natural_cover_3k + contag_landscape_3k + np_landscape_3k + ndvi_mean + ebi_max + ndwi_mean + temp_max + rain_value + moon_illu + period, data = spatial, approach = "AIC", kernel = "tricube", adaptive = F, dMat = dist_matrix)
...
#Model 27 was the lowest AIC - no EBI#

DeVar <- "n"

IndVar <- c("contag_landscape_325" , "np_landscape_325" , "temp_max" , "rain_value" , "moon_illu" , "month")

gwr.res <- gwr.basic(n ~ contag_landscape_325 + np_landscape_325 + temp_max + rain_value + moon_illu + month, data = spatial,  bw = BD_sel, adaptive = F, kernel = "tricube")
summary(gwr.res$lm)

model <- as.data.frame(gwr.res$SDF)

model$site <- cbind(data_og$site)
model_grouped <- group_by(model, site) %>% 
  summarise_all(., mean) %>% 
  dplyr::select(site, all_of(IndVar), everything())
...
ggplot(model, aes(x = site, y = Local_R2)) +
  ...
  ggsave(getDataPath("Figures", "03.05.2022_gwrr2valuespersite_birdmodel.tiff"))


df_gather <- pivot_longer(model_grouped, cols = c(2:4), names_to = "coefficient_name", values_to = "coefficient_value") %>% 
...
ggplot(df_gather, aes(x = site, y = coefficient_value, fill = coefficient_name)) +
  ...
  labs(title = "Coefficients per point - Night model", x = "Site", y = "Coefficients Value", fill = "Variables")
  ggsave(getDataPath("Figures", "03.05.2022_Coefficients_birdmodel.tiff"))
  
#3k model ----
  global_model <- lm(n ~ ca_class_1_3k + ca_class_9_3k + np_landscape_3k + temp_max + rain_value + moon_illu + bvg_char + month, data = data_og)
  ...
  spatial$bvg_char <- as.factor(spatial$bvg_char)
  lm.bp <- bptest(n ~ ca_class_1_3k + ca_class_9_3k + np_landscape_3k + temp_max + rain_value + moon_illu + bvg_char + month, data = data_og, studentize = T)
  ...
  IndVar <- c("np_landscape_3k", "temp_max", "rain_value", "month")
  ...
  #Global Model#
  #Selecting bandwidth#
  # BD_sel <- bw.ggwr(n ~ ca_class_1_3k, data = spatial, approach = "AIC", kernel = "tricube", adaptive = F, dMat = dist_matrix)
  BD_sel <- bw.ggwr(n ~ np_landscape_3k + temp_max + rain_value + moon_illu + month, data = spatial, approach = "AIC", kernel = "tricube", adaptive = F, dMat = dist_matrix)
  ...
  gwr.res <- gwr.basic(n ~ np_landscape_3k + temp_max + rain_value + month, data = spatial,  bw = BD_sel, adaptive = F, kernel = "tricube")
  summary(gwr.res$lm)
  ...
  ggplot(model, aes(x = site, y = Local_R2)) +
    ...
    theme(panel.border = element_blank(), legend.position = "none") +
    labs(title = "Local R2 values per point", x = "Point", y = "Local R2", fill = "Period")
  ggsave(getDataPath("Figures", "03.05.2022_gwrr2valuespersite_birdmodel_3k.tiff"))
  ...
  ggplot(df_gather, aes(x = site, y = coefficient_value, fill = coefficient_name)) +
    ...
    #theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
    labs(title = "Coefficients per point - Night model", x = "Site", y = "Coefficients Value", fill = "Variables")
  ggsave(getDataPath("Figures", "03.05.2022_Coefficients_birdmodel_3k.tiff"))
