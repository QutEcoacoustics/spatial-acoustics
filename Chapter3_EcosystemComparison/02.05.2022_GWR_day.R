...
data_og <- filter(data_og, period == "dawn")
...
#After checking the time series, it is clear that tags birdfrog don't have more than 5 motifs across sites, therefore they will be excluded from analysis from now on - tags kept = bird, birdinsect, insect

data_og <- filter(data_og, RFclass == "bird" | RFclass == "birdinsect" | RFclass == "insect")
...
global_model <- lm(n ~ ca_class_1_3k + ca_class_9_3k + np_landscape_3k + temp_max + rain_value + moon_illu + bvg_char, data = data_og)
...
lm.bp <- bptest(n ~ ca_class_1_3k + ca_class_9_3k + np_landscape_3k + temp_max + rain_value + moon_illu + bvg_char, data = data_og, studentize = T)
...
IndVar <- c("np_landscape_3k", "temp_max", "rain_value", "moon_illu")
...
#Global Model#
#Selecting bandwidth#
# BD_sel <- bw.ggwr(n ~ moon_illu, data = spatial, approach = "AIC", kernel = "tricube", adaptive = F, dMat = dist_matrix)
BD_sel <- bw.ggwr(n ~ np_landscape_3k + temp_max + rain_value + moon_illu, data = spatial, approach = "AIC", kernel = "tricube", adaptive = F, dMat = dist_matrix)
...
ggplot(model, aes(x = site, y = Local_R2)) +
  ...
  ggsave(getDataPath("Figures", "02.05.2022_gwrr2valuespersite_dawnmodel.tiff"))
...
ggplot(df_gather, aes(x = site, y = coefficient_value, fill = coefficient_name)) +
  ...
  ggsave(getDataPath("Figures", "02.05.2022_Coefficients_dawnmodel.tiff"))
