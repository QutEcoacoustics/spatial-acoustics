...
data_og <- filter(data_og, period == "dusk")
...
#After checking the time series, it is clear that tags bat, batbirdinsect, batfroginsect, birdmammal and froginsectmammal don't have more than 5 motifs across sites, therefore they will be excluded from analysis from now on - tags kept = bird, birdinsect, insect
...
IndVar <- c("np_landscape_3k", "temp_max", "rain_value", "moon_illu", "month")
...
gwr.res <- gwr.basic(n ~ np_landscape_3k + temp_max + rain_value + moon_illu  + month, data = spatial,  bw = BD_sel, adaptive = F, kernel = "tricube")
summary(gwr.res$lm)
...
ggplot(model, aes(x = site, y = Local_R2)) +
  ...
  ggsave(getDataPath("Figures", "02.05.2022_gwrr2valuespersite_daymodel.tiff"))
...
ggplot(df_gather, aes(x = site, y = coefficient_value, fill = coefficient_name)) +
  ...
  labs(title = "Coefficients per point - Day model", x = "Site", y = "Coefficients Value", fill = "Variables")
  ggsave(getDataPath("Figures", "02.05.2022_Coefficients_daymodel.tiff"))
